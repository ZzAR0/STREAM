...
def _resize_by_channels(img, shape, interpolation=cv2.INTER_LINEAR):
    if img.shape[2] == 2:
        return resize_mv(img, shape, interpolation)
    return cv2.resize(img, shape, interpolation)


class StreamGroupScale(object):
    """Scale iframe/residual/mv with identical output size."""
    ...
    def __call__(self, sample):
        return {
            key: [_resize_by_channels(img, self._size, cv2.INTER_LINEAR) for img in imgs]
            for key, imgs in sample.items()
        }


class StreamGroupCenterCrop(object):
    """Center crop all STREAM modalities with the same coordinates."""

    def __init__(self, size):
        self._size = size

    def __call__(self, sample):
        h, w, _ = sample["iframe"][0].shape
        hs = (h - self._size) // 2
        ws = (w - self._size) // 2
        return {
            key: [img[hs:hs + self._size, ws:ws + self._size] for img in imgs]
            for key, imgs in sample.items()
        }


class StreamGroupMultiScaleCrop(object):
    """Random crop all STREAM modalities using one shared crop box."""
    ...
    def __call__(self, sample):
        im_size = sample["iframe"][0].shape
        crop_w, crop_h, offset_w, offset_h = self._sample_crop_size(im_size)
        out_size = (self.input_size[0], self.input_size[1])
        ret = {}
        for key, imgs in sample.items():
            cropped = [img[offset_w:offset_w + crop_w, offset_h:offset_h + crop_h] for img in imgs]
            ret[key] = [_resize_by_channels(img, out_size, cv2.INTER_LINEAR) for img in cropped]
        return ret
...
class StreamRandomHorizontalFlip(object):
    """Flip all STREAM modalities together; only MV x displacement changes sign."""
    ...
    def __call__(self, sample):
        ...
        ret = {}
        for key, imgs in sample.items():
            flipped = [img[:, ::-1, :].astype(np.int32) for img in imgs]
            if key == "mv":
                for img in flipped:
                    img -= 128
                    img[..., 0] *= -1
